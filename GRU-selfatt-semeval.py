...
filepath = '/home/wenger/zhengxitian/atGRU/semeval2014/'
# filename1 = '/home/wenger/zhengxitian/atGRU/semeval2014/restaurant_train.txt'
# filename2 = '/home/wenger/zhengxitian/atGRU/semeval2014/laptop_train.txt'
# filename3 = '/home/wenger/zhengxitian/atGRU/semeval2014/restaurant_test.txt'
# filename4 = '/home/wenger/zhengxitian/atGRU/semeval2014/laptop_test.txt'
...
def __preProduceFile__(filename):
    ...
    if 'train' in str(fopen):
        for eachLine in fopen.readlines():
            ...
            if Y_train is None:
                if score > 0:
                    Y_train = np.array([score,0])
                elif score < 0:
                    Y_train = np.array([0,score])
                else:
                    Y_train = np.array([0,0])
            else:
                if score > 0:
                    Y_train = np.row_stack((Y_train, np.array([score,0])))
                elif score < 0:
                    Y_train = np.row_stack((Y_train, np.array([0,score])))
                else:
                    Y_train = np.row_stack((Y_train, np.array([0,0])))
    # Test sets
    else:
        for eachLine in fopen.readlines():
            labels = 0
            ...
            if Y_test is None:
                if score > 0:
                    Y_test = np.array([score,0])
                elif score < 0:
                    Y_test = np.array([0,score])
                else:
                    Y_test = np.array([0,0])
            else:
                if score > 0:
                    Y_test = np.row_stack((Y_test, np.array([score,0])))
                elif score < 0:
                    Y_test = np.row_stack((Y_test, np.array([0,score])))
                else:
                    Y_test = np.row_stack((Y_test, np.array([0,0])))
...
print('X train:', X_train[100:102])
print('Y train:', Y_train[100:102])
print('X test:', X_test[100:102])
print('Y test:', Y_test[100:102])
...
# Save model
print ('Saving model...')
model.save('GRU-selfAtt-20-semeval-200d')
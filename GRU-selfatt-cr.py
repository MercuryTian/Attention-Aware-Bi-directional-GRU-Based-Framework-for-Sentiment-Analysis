        ...
        if '##' in eachLine:
            if eachLine.startswith('##'):
                ...
                if Y is None:
                    ...
                    if score > 0:
                        Y = np.row_stack((Y, np.array([score,0])))
                    elif score < 0:
                        Y = np.row_stack((Y, np.array([0,score])))
                if score != 0:
                    X.append(tmp[1])
                    # print('this X is:', tmp[1])
    fopen.close()
...
print('X[100]:', X[100])
print('Y[100]:', Y[100])
print('X tarin:', X_train[3600])
print('Y train:', Y_train[3600])
...
# Save model
print ('Saving model...')
model.save('GRU-selfAtt-20-cr-200d')
